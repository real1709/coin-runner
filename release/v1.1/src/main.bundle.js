(function () {
  ...
  const IP_THEME = {
    runner: {
      name: "회사IP_러너",
      color: "#22384d",
      imageSrc: "./assets/runner_right_foot.png",
      frameImages: [
        "./assets/runner_right_foot.png",
        "./assets/runner_left_foot.png"
      ],
      fps: 7.5
    },
    holeEnemy: {
  ...
  function drawRunnerAnimated(player) {
    const frameImages = IP_THEME.runner.frameImages || [];
    if (frameImages.length > 0) {
      const fps = Math.max(1, IP_THEME.runner.fps || 8);
      const frameIndex = Math.floor(player.animTime * fps) % frameImages.length;
      return drawCroppedSprite(
        "runnerFrame" + frameIndex,
        player.x,
        player.y,
        player.width,
        player.height
  ...
  function getPlayerHitbox() {
    const player = runtime.player;
    return {
      left: player.x + player.width * 0.24,
      right: player.x + player.width * 0.76,
      top: player.y + player.height * 0.2,
      bottom: player.y + player.height * 0.94
    };
  }
  ...
  function updatePlayer(dt) {
    ...
    player.bob = Math.sin(player.animTime * 10.6) * (1.6 + feel * 4.2);
  ...
  function renderPlayer() {
    ...
    const shadowW = 17 + runtime.scrollSpeedNorm * 28;
    const shadowH = 6 + runtime.scrollSpeedNorm * 8;
    ctx.fillStyle = "rgba(12, 20, 35, 0.26)";
    ctx.beginPath();
    ctx.ellipse(playerCenterX, shadowY, shadowW, shadowH, 0, 0, Math.PI * 2);
    ctx.fill();

    const color = runtime.shieldTimer > 0 ? "#ffd364" : IP_THEME.runner.color;
    ctx.save();
    ctx.translate(playerCenterX, player.y + player.height * 0.82);
    ctx.rotate(player.lean * 0.06);
    ctx.translate(-playerCenterX, -(player.y + player.height * 0.82));
    const rendered = drawRunnerAnimated(player);
    if (!rendered) {
      drawIpSprite(
        "runner",
        player.x,
