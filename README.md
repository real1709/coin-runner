...
단, 공용 랭킹(팀 전체 공유 순위)은 서버 API를 쓰는 `http://` 실행에서만 동작하며 `file://` 실행은 기기 로컬 랭킹으로 동작합니다.
또한 GitHub Pages 같은 정적 호스팅은 서버 API를 실행하지 못하므로 공용 랭킹이 저장되지 않습니다.
...
- 랭킹 저장: 서버 실행(`npm run serve`) 시 공용 랭킹 API(`/api/leaderboard`)에 저장/조회
- 화면 연출: 타이틀/결과 이미지, 타이틀 BGM, 카운트다운/GO/인게임 사운드, 좌우 배경 장식 오브젝트
- 마케팅 기능: 공유 버튼
- 텔레메트리 이벤트:
  - `session_start`
  - `stage_start`
